...
function Node({
  ...
  // celadon completion ring follows a beat later
  ...
  return (
    <motion.g style={reduced ? undefined : { opacity }}>
      <motion.circle
        ...
        r={4}
        fill="var(--fire)"
        style={reduced ? undefined : { scale, transformOrigin: `${x}px ${LINE_Y}px` }}
      />
      <motion.circle
        cx={x}
        cy={LINE_Y}
...
/* small ember travelling the drawn path forever — the factory keeps going */
function Traveller({
  pathRef,
  progress,
}: {
  ...
  return (
    <g ref={dotRef} opacity={0}>
      <circle r={6} fill="rgba(217,131,42,0.30)" />
      <circle r={2.5} fill="var(--fire)" />
    </g>
  )
}
...
export default function S03_Pipeline() {
  ...
  return (
    ...
    >
      ...
      >
        <motion.p
          className="eyebrow mb-20 px-6 md:px-16"
          style={{ color: 'var(--cel)' }}
        ...
        <motion.div style={reduced ? undefined : { x }} className="will-change-transform">
          ...
          >
            ...
            />
            {STAGES.map((_, i) => (
              <Node key={STAGES[i]} i={i} progress={pathLength} reduced={reduced} />
            ))}
